...
/**
 * Master的Curator LeaderLatchListener版本
 ...
 * 5、挂掉时，会主动释放Master节点
 */
public class MasterCuratorLatch implements Closeable, LeaderLatchListener {
    private static final Logger logger = LoggerFactory.getLogger(MasterCuratorLatch.class);
    ...
    public MasterCuratorLatch(String hostPort, RetryPolicy retryPolicy){
        this.client = CuratorFrameworkFactory.newClient(hostPort, retryPolicy);
        this.leaderLatch = new LeaderLatch(this.client, "/master", masterId);
    ...
    public void isLeader() {
        ...
        for(ChildData workerCache :workersList){
			...
			try {
				...
				logger.warn("assignTaskCache start error: "+e.getMessage());
			}
    ...
    public static void main (String[] args) {
        try{
            MasterCuratorLatch master = new MasterCuratorLatch("localhost:2181", new ExponentialBackoffRetry(1000, 5));
            master.startZK();
